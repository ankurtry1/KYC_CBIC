...
import { getFeaturedOfficers, getOfficerMetrics } from "@/lib/officers/load";

export default async function HomePage(): Promise<JSX.Element> {
  const [metrics, featured] = await Promise.all([getOfficerMetrics(), getFeaturedOfficers(6)]);

  return (
    <main className="min-h-screen bg-surface">
      <AppTopNav />

      <section className="relative overflow-hidden border-b border-slate-200/80 bg-mesh-soft">
      ...
      <section className="mx-auto w-full max-w-7xl px-4 py-10 md:px-8">
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <AnimatedStatCard label="Total Officers" value={metrics.total_officers} hint="Parsed profiles" delay={0} />
          <AnimatedStatCard
            ...
            delay={0.05}
          />
          <AnimatedStatCard label="Cadres Covered" value={metrics.cadres_covered} hint="DR / PA / PC / PE" delay={0.1} />
          <AnimatedStatCard
